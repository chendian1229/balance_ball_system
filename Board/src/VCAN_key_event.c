...
void deal_key_event()
{
    KEY_MSG_t keymsg;  //定义一个按键消息结构体（按键编号和按键状态）

    while(get_key_msg(& keymsg))     //获得按键就进行处理
    {
        if(keymsg.status == KEY_DOWN)  //按键状态为按下
        {
            switch(keymsg.key)     //判断按键编号 是哪个按键按下
            {
            case KEY_U:             //上键按下
                var_value(VAR_ADD);  //对变量的加减进行处理  VAR_ADD表示加1
                break;
  
            case KEY_D:             //下键按下
                var_value(VAR_SUB);  //对变量的加减进行处理  VAR_SUB表示减1
                break;
        ...
        else if(keymsg.status == KEY_HOLD)  //按键状态为长按按键(用于定时按键扫描)
        {
            switch(keymsg.key)
            {
            case KEY_U:
